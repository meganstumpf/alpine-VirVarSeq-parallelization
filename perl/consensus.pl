...
my $sample = $sample_file;   #my @samples = Files::openFile($sample_file);
my $num_of_samples = scalar(1);    #my $num_of_samples = scalar(@samples);
Log::screen($0, "Samples number [$num_of_samples]");

# Calculate consensus file for each sample
Log::screen($0, "2.4 Calculate consensus ...");
#for (my $i=0; $i < $num_of_samples; ++$i){
#my $sample = $samples[$i];
	Log::screen($0, "Sample [$sample]");
	#print @sample;
