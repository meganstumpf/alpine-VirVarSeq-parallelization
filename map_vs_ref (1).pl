#!/usr/bin/env perl
use lib '/projects/.xsede.org/mstumpf/.perl5/lib/perl5';
...
my $sample = $sample_file;    #Files::openFile($sample_file);
Log::screen($0, "Samples names/file [$sample]");
my $num_of_samples = scalar(1);        #scalar(@samples);
...
#my $sample = $samples; 	#$samples[$ia];
	...
	if($mapping eq "single"){
		...
		Log::screen($0,"bwa mapping samfile[$samfile], [$rundir], [$read1] ...");
	}
